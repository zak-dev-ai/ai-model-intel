// api/blog.js
// Simple Vercel serverless function
// Returns generated blog articles

const SUPABASE_URL = 'https://jkoxrftlslylfmugjomd.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imprb3hyZnRsc2x5bGZtdWdqb21kIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc4MjAzNTEsImV4cCI6MjA5MzM5NjM1MX0.1LVo1a2K2V8QZPX94bVTBmzIxUskKp0mR7RmNwgOdcI';
const MODELS_API = 'https://ai-model-intel.zak-media-ai.workers.dev/pricing';

module.exports = async (req, res) => {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Cache-Control', 'public, max-age=1800, s-maxage=1800');
  
  if (req.method === 'OPTIONS') {
    res.status(200).end();
    return;
  }

  try {
    let articles = [];

    // 1. Try loading from Supabase cache
    try {
      const r = await fetch(
        `${SUPABASE_URL}/rest/v1/analytics?event=eq.blog_articles_json&select=value&limit=1&order=created_at.desc`,
        { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
      );
      const data = await r.json();
      if (data && data.length > 0 && data[0].value) {
        articles = typeof data[0].value === 'string' ? JSON.parse(data[0].value) : data[0].value;
      }
    } catch {}

    // 2. Check if articles need refresh (new day)
    const today = new Date().toISOString().split('T')[0];
    const needsRefresh = !articles.length || (articles[0] && articles[0]._date !== today);
    
    if (needsRefresh) {
      articles = await generateArticles(today);
      // Cache in Supabase
      try {
        await fetch(`${SUPABASE_URL}/rest/v1/analytics`, {
          method: 'POST',
          headers: {
            'apikey': SUPABASE_KEY,
            'Authorization': `Bearer ${SUPABASE_KEY}`,
            'Content-Type': 'application/json',
            'Prefer': 'return=minimal'
          },
          body: JSON.stringify({
            event: 'blog_articles_json',
            page: '/blog',
            value: JSON.stringify(articles),
            created_at: new Date().toISOString()
          })
        });
      } catch {}
    }

    res.status(200).json({ articles });
  } catch (e) {
    res.status(200).json({ articles: [] });
  }
};

async function generateArticles(today) {
  const dateStr = new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  const articles = [];
  let modelData = [];

  try {
    const r = await fetch(MODELS_API);
    const d = await r.json();
    modelData = d.pricing || [];
  } catch {}

  let rssHeadlines = [];
  try {
    const rss = await fetch('https://hnrss.org/frontpage?count=10');
    const text = await rss.text();
    const matches = text.match(/<title><!\[CDATA\[([^\]]+)\]\]><\/title>/g) || [];
    rssHeadlines = matches.slice(0, 5).map(m =>
      m.replace('<title><![CDATA[', '').replace(']]></title>', '')
    ).filter(h => !/^(Ask|Show) HN/.test(h));
  } catch {}

  if (modelData.length > 0) {
    const top3 = modelData.slice(0, 3);
    articles.push({
      _date: today, id: 101, cat: 'pricing', emoji: '💰', tag: 'PRICING',
      title: `AI Price Watch — ${dateStr}`,
      excerpt: `Cheapest: ${top3[0]?.model?.split(': ').pop() || 'N/A'} at $${top3[0]?.input_cost_per_mtok || '?'}/1M input.`,
      date: dateStr, read: '3 min', views: '0.8K',
      featured: false, isNew: true, isAI: true,
      bg: 'linear-gradient(135deg,#001a0a,#003018)',
      content: `<h2>Top Budget Models Today</h2>
        <div class="price-table">${top3.map(m => `<div class="price-row"><span>${m.model?.split(': ').pop() || m.model}</span><span class="pv">$${m.input_cost_per_mtok}/1M</span></div>`).join('')}</div>
        <div class="highlight">Route simple tasks to budget models ($0.02-0.05/1M). Save premium for reasoning. Cuts API bills 60-80%.</div>
        <p>Full rankings: <a href="https://www.aimodelranks.live/compare" style="color:var(--cyan)">aimodelranks.live</a></p>`
    });
  }

  articles.push({
    _date: today, id: 102, cat: 'news', emoji: '📰', tag: 'AI NEWS',
    title: `AI Briefing — ${dateStr}`,
    excerpt: rssHeadlines.length > 0 ? rssHeadlines.slice(0, 2).join(', ').substring(0, 120) : 'AI industry updates and trends.',
    date: dateStr, read: '3 min', views: '0.5K',
    featured: false, isNew: true, isAI: true,
    bg: 'linear-gradient(135deg,#001428,#002850)',
    content: `<h2>Today in AI</h2>
      ${rssHeadlines.length > 0
        ? rssHeadlines.map((h, i) => `<h2>${i+1}. ${h}</h2><p>Trending on Hacker News.</p>`).join('')
        : '<p>AI pricing continues dropping. New models at lower price points every week.</p>'}
      <div class="highlight">Bookmark <a href="https://www.aimodelranks.live" style="color:var(--cyan)">aimodelranks.live</a> for daily AI intelligence.</div>`
  });

  articles.push({
    _date: today, id: 103, cat: 'tools', emoji: '🛠', tag: 'AI TOOLS',
    title: `AI Tool Spotlight — ${dateStr}`,
    excerpt: 'Community-voted AI tools you should know about this week.',
    date: dateStr, read: '3 min', views: '0.6K',
    featured: false, isNew: true, isAI: false,
    bg: 'linear-gradient(135deg,#0a0a28,#1a1a50)',
    content: `<h2>This Week's Standouts</h2>
      <p>Browse and vote at <a href="https://www.aimodelranks.live/tools" style="color:var(--cyan)">aimodelranks.live/tools</a>.</p>
      <h2>Code Assistants</h2>
      <p>Cursor leads for codebase-aware AI editing. Claude excels at reasoning. Copilot is most widely adopted.</p>
      <div class="highlight">Full directory: <a href="https://www.aimodelranks.live/tools" style="color:var(--cyan)">aimodelranks.live/tools</a> with 20+ tools and community voting.</div>`
  });

  return articles;
}