...
// ══════════════════════════════════════════════════════════════
// COMPREHENSIVE MOBILE CSS FIX — injected on all pages
// Fixes: nav, cards, layout, images, buttons, overflow
// ══════════════════════════════════════════════════════════════
(function injectMobileFixes() {
  if (document.getElementById('octopus-mobile-fixes')) return;
  var css = document.createElement('style');
  css.id = 'octopus-mobile-fixes';
  css.textContent = [
    '',
    '/* === MOBILE & TABLET FIXES === */',
    '',
    ...
    '',
    '  /* ── ANN BAR: compact ── */',
    '  .ann{padding:6px 10px;font-size:10px;gap:6px;flex-wrap:wrap;text-align:center;}',
    '  .ann strong{display:block;width:100%;text-align:center;}',
    '  .ann-sep{display:none;}',
    '  .ann a{display:none;}',
    '',
    '  /* ── NAV: slim header (works on ALL pages — .nav-top, .nav-inner) ── */',
    '  .nav-top,.nav-inner{padding:0 10px!important;height:54px!important;gap:8px!important;}',
    '  .nav-logo{font-size:12px;letter-spacing:0;}',
    '  .nav-sw,.nav-search-wrap{flex:1;max-width:200px;}',
    '  .nav-search{padding:8px 8px 8px 30px;font-size:13px;}',
    '  .sico{left:8px;font-size:14px;}',
    '  .nav-right,.nav-links{display:none!important;}',
    '  .burger{display:block!important;font-size:24px;padding:4px 8px;margin-right:4px;}',
    '  .btn-api{display:none;}',
    '',
    '  /* ── NAV TABS: smooth horizontal scroll + snap (works on ALL pages) ── */',
    '  .nav-tabs,.mob-nav{padding:0 4px;overflow-x:auto;-webkit-overflow-scrolling:touch;scroll-snap-type:x proximity;gap:0;}',
    '  .ntab{padding:9px 12px;font-size:11px;flex-shrink:0;scroll-snap-align:start;}',
    '  .mob-nav a{padding:6px 10px;font-size:11px;flex-shrink:0;scroll-snap-align:start;}',
    '',
    '  /* ── MOBILE MENU (burger overlay) ── */',
    '  .mobile-menu{-webkit-overflow-scrolling:touch;padding:70px 16px 30px;z-index:1002;}',
    '  .mobile-menu a{font-size:18px;padding:16px 8px;min-height:52px;align-items:center;}',
    '  .mobile-menu .mob-close{top:12px;right:16px;font-size:30px;}',
    '',
    '  /* ── MODEL PRICE CARDS (marquee row 1) ── */',
    '  .mc{width:155px!important;flex-shrink:0!important;padding:10px 12px!important;}',
    '  .mc-lbl{font-size:9px!important;}',
    '  .mc-val{font-size:15px!important;}',
    '  .mc-sub{font-size:10px!important;}',
    '  .mq-track{gap:0!important;animation:mq var(--spd,60s) linear infinite!important;-webkit-animation:mq var(--spd,60s) linear infinite!important;}',
    '  .mq-track>*{flex-shrink:0!important;}',
    '',
    '  /* ── TICKER ROW 2 ── */',
    '  .tk-label,.tl-label{font-size:9px;padding:0 8px;}',
    '  .tick{font-size:10px;padding:0 10px;}',
    '  .ttick{font-size:10px;padding:0 10px;}',
    '',
    '  /* ── SPOTLIGHT CAROUSEL (row 3) — cards uniform + compact ── */',
    '  .spot-hdr{padding:6px 10px;flex-wrap:wrap;gap:4px;}',
    '  .spot-hdr-l{font-size:9px;}',
    '  .spot-hdr-r{font-size:9px;}',
    '  .scard,.scard.tier-premium,.scard.tier-growth,.scard.tier-starter,.scard.tier-empty,.scard.tier-featured{',
    '    width:170px!important;min-width:170px!important;margin:0 6px!important;flex-shrink:0!important;',
    '    transition:none!important;',
    '  }',
    '  .scard .sc-body{padding:10px!important;}',
    '  .scard .sc-badge{font-size:7px!important;padding:2px 6px!important;margin-bottom:6px!important;}',
    '  .scard .sc-name,.scard.tier-premium .sc-name,.scard.tier-growth .sc-name,.scard.tier-starter .sc-name{font-size:11px!important;}',
    '  .scard .sc-desc,.scard.tier-premium .sc-desc,.scard.tier-growth .sc-desc,.scard.tier-starter .sc-desc{font-size:10px!important;}',
    '  .scard .sc-cta{padding:6px 10px!important;font-size:10px!important;}',
    '  .scard img{width:36px!important;height:36px!important;}',
    '  .scard .sc-empty-ico{font-size:18px!important;}',
    '  .scard .sc-empty-price{font-size:12px!important;}',
    '',
    '  /* ── TOOL CARDS (row 5 marquee) ── */',
    '  .tool-hdr{padding:6px 10px;}',
    '  .tool-hdr-l,.tool-hdr-r{font-size:9px;}',
    '  .tc,.tool-card{width:165px!important;min-width:165px!important;flex-shrink:0!important;margin:0 4px!important;}',
    '  .tc-thumb{height:70px!important;}',
    '  .tc-body{padding:8px!important;}',
    '  .tc-name{font-size:10px!important;}',
    '  .tc-cat{font-size:8px!important;}',
    '  .tc-pr,.tc-pricing{font-size:8px!important;padding:2px 5px!important;}',
    '  .tc-votes,.vote-btn{font-size:9px!important;}',
    '  .tc img{width:36px!important;height:36px!important;}',
    '  .tc-rank{top:4px;left:4px;font-size:7px;padding:1px 4px;}',
    '',
    '  /* ── HERO: compact ── */',
    '  .hero{padding:28px 12px 20px;}',
    '  .hero h1{font-size:24px!important;line-height:1.15;}',
    '  .hero-sub{font-size:13px!important;max-width:100%;} .hero-sub br{display:none;}',
    '  .hero-badge{font-size:9px;padding:4px 10px;margin-bottom:12px;}',
    '  .hero-btns{gap:8px;margin-bottom:20px;}',
    '  .btn-hp,.btn-hs,.btn-ha{padding:10px 16px;font-size:12px;white-space:nowrap;}',
    '  .hero-stats{flex-wrap:wrap;border-radius:8px;}',
    '  .hstat{padding:10px 8px;min-width:45%;flex:0 1 auto;border-right:1px solid var(--border);border-bottom:1px solid var(--border);}',
    '  .hstat:nth-child(odd){border-right:1px solid var(--border);}',
    '  .hstat:nth-child(n+3){border-bottom:none;}',
    '  .hstat-num{font-size:18px!important;}',
    '  .hstat-lbl{font-size:8px;}',
    '',
    '  /* ── TOP 6 GRID: 2×3 ── */',
    '  .top6-section{padding:12px 8px;}',
    '  .top6-hdr{margin-bottom:8px;}',
    '  .top6-title{font-size:10px;}',
    '  .top6-sub{font-size:9px;}',
    '  .top6-grid{grid-template-columns:repeat(3,1fr)!important;gap:6px;}',
    '  .top6-card{min-height:110px;}',
    '  .t6-name{font-size:9px!important;}',
    '  .t6-desc{font-size:8px!important;-webkit-line-clamp:2;}',
    '  .t6-body{padding:7px!important;}',
    '  .t6-ico{font-size:14px;}',
    '  .t6-badge{font-size:7px;}',
    '  .t6-cta{font-size:8px!important;padding:3px!important;}',
    '  .t6-expiry{font-size:7px;}',
    '',
    '  /* ── AD SHOWCASE (tier-3) ── */',
    '  .ad-showcase{padding:12px 8px;}',
    '  .ashow-hdr{flex-wrap:wrap;gap:4px;margin-bottom:10px;}',
    '  .ashow-title{font-size:10px;}',
    '  .ashow-sub{font-size:9px;}',
    '  .tier-row{grid-template-columns:1fr!important;gap:8px;}',
    '  .adshow-card .ac-body{padding:14px;min-height:auto;}',
    '  .adshow-card .ac-name{font-size:13px;}',
    '  .adshow-card .ac-desc{font-size:11px;}',
    '  .adshow-card .ac-cta{padding:8px 14px;font-size:11px;}',
    '  .adshow-card.empty .ac-body{min-height:140px;}',
    '',
    '  /* ── HOME TOOLS GRID: 2 cols (3 feels cramped) ── */',
    '  .tools-section{padding:14px 0;}',
    '  .ts-hdr{flex-wrap:wrap;gap:4px;}',
    '  .ts-title{font-size:12px;}',
    '  .ts-link{font-size:10px;}',
    '  .cat-pills{gap:4px;padding:0 0 8px;}',
    '  .cat-pill{padding:4px 9px;font-size:10px;}',
    '  .ts-toolbar{flex-wrap:wrap;gap:6px;}',
    '  .ts-count{font-size:10px;}',
    '  .ts-sort select{font-size:10px;padding:5px 8px;}',
    '  .home-tools-grid{grid-template-columns:repeat(2,1fr)!important;gap:6px;}',
    '  .ht-img-wrap{height:60px!important;}',
    '  .ht-img{width:32px!important;height:32px!important;}',
    '  .ht-img-fallback{font-size:24px!important;}',
    '  .ht-body{padding:7px!important;}',
    '  .ht-name{font-size:10px!important;}',
    '  .ht-tagline{font-size:9px!important;-webkit-line-clamp:2!important;}',
    '  .ht-meta{flex-wrap:wrap!important;gap:3px!important;}',
    '  .ht-pricing{font-size:8px!important;padding:2px 4px!important;}',
    '  .ht-votes{font-size:8px!important;}',
    '  .ht-rank{font-size:7px!important;padding:1px 4px!important;}',
    '  .ht-new-badge{font-size:6px!important;}',
    '  .ht-card.featured::after{font-size:7px!important;padding:1px 4px!important;top:4px;right:4px;}',
    '',
    '  /* ── PAGE GRID + SIDEBAR ── */',
    ...
    '',
    '  /* ── STATS STRIP ── */',
    '  .stats-strip{padding:6px 0;gap:0;}',
    '  .ss{padding:0 8px;font-size:10px;gap:3px;}',
    '',
    '  /* ── MOBILE ADS (shown at tablet breakpoint) ── */',
    '  .mob-ads{display:block!important;}',
    '  .mob-ad{gap:8px;margin-bottom:8px;}',
    '  .mob-ad-bar{width:3px;}',
    '  .mob-ad-body{padding:8px;}',
    '  .mob-ad-badge{font-size:7px;}',
    '  .mob-ad-name{font-size:11px;}',
    '  .mob-ad-desc{font-size:9px;}',
    '  .mob-ad-btn{padding:6px 10px;font-size:9px;margin-right:8px;}',
    '',
    '  /* ── TABLE: compact ── */',
    '  .tbl-wrap{border-radius:8px;}',
    '  th{padding:8px 6px;font-size:8px;letter-spacing:0.3px;}',
    '  td{padding:8px 6px;}',
    '  td.td-n{min-width:120px;}',
    '  .pm{font-size:10px;}',
    '  .ps{font-size:8px;max-width:100px;}',
    '  .pico{width:22px;height:22px;border-radius:5px;font-size:10px;}',
    '  .nw{gap:5px;}',
    '  tr td:nth-child(7),thead th:nth-child(7){display:none;}',
    '  tr td:nth-child(8),thead th:nth-child(8){display:none;}',
    '  tr td:nth-child(9),thead th:nth-child(9){display:none;}',
    '  tr td:nth-child(10),thead th:nth-child(10){display:none;}',
    '  tr td:nth-child(11),thead th:nth-child(11){display:none;}',
    '',
    '  /* ── COMPARISON PANEL ── */',
    '  .cmp-cols{grid-template-columns:1fr!important;gap:8px;}',
    '  .chips{flex-wrap:wrap;}',
    '',
    '  /* ── PROMO AD ── */',
    '  .promo-ad{padding:12px;}',
    '  .promo-title{font-size:10px;}',
    '  .promo-sub{font-size:10px;}',
    '  .promo-price{font-size:16px;}',
    '',
    '  /* ── OUTER CONTAINER ── */',
    '  .outer{padding:0 10px;}',
    '',
    '  /* ── TOAST ── */',
    '  #toast{font-size:10px;padding:7px 14px;bottom:16px;}',
    '',
    '  /* ── FOOTER (if any) ── */',
    '  footer{padding:16px 10px;font-size:10px;}',
    '',
    '}',
    '',
    '/* === EXTRA SMALL (375px phones) === */',
    '@media(max-width:420px){',
    '  .hero h1{font-size:20px!important;}',
    '  .hero-stats{flex-wrap:wrap;}',
    '  .mcards-track{gap:0!important;}',
    '  .mc{width:130px!important;padding:8px 10px!important;}',
    '  .mc-val{font-size:13px!important;}',
    '  .scard,.scard.tier-premium,.scard.tier-growth,.scard.tier-starter,.scard.tier-empty{width:140px!important;min-width:140px!important;}',
    '  .tc,.tool-card{width:135px!important;min-width:135px!important;}',
    '  .top6-grid{grid-template-columns:repeat(2,1fr)!important;}',
    '  .home-tools-grid{grid-template-columns:repeat(2,1fr)!important;}',
    '  .nav-logo{font-size:10px;}',
    '  .nav-sw{max-width:140px;}',
    '  .nav-search{font-size:12px;}',
    '  .mobile-menu a{font-size:16px;padding:14px 8px;}',
    '}',
  ].join('\n');
